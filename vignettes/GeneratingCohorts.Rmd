...
saveCohortDefinitionSet(cohortDefinitionSet = cohortDefinitionSet,
                        settingsFolder = file.path(packageRoot, "inst/settings"),
...
Your study package can later re-construct the cohortDefinitionSet by reading in these resources into the data frame structure described earlier in this vignette.
...
Next we'll create the tables to store the cohort and the cohort statistics. Since we have chosen to `generateStats`, when creating the cohort tables, we need to set `createCohortStatsTables = TRUE` to create the proper tables to hold the results. Then we can generate the cohorts.
...
Once you have exported your cohort statistics, you can optionally drop the statstics tables by using the following command:
